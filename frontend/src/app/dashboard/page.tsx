...
const Icons = {
  ...
  missing: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414A1 1 0 0120 9.414V19a2 2 0 01-2 2z" />
    </svg>
  ),
  credit: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
...
export default function DashboardPage() {
  ...
  }, [canSubmit, statementFile, ledgerFile, vendorName]);
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-0.5">
                Match vendor statements against your AP ledger and surface exceptions instantly.
              </p>
            </div>
            {stage === "done" && (
              <button
                onClick={reset}
                className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-sm hover:bg-slate-50 transition-colors"
              >
                {Icons.refresh}
                New reconciliation
              </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 animate-fade-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
                  ...
                  <DropZone
                    label="Drop vendor statement here"
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
                  ...
                  <DropZone
                    label="Drop AP ledger export here"
                    hint="Drag & drop or click to browse"
                ...
                <div className="space-y-4">
                  {s.exception_count === 0 ? (
                    ...
                    <>
                      <ExceptionsTable
                        title="Amount Mismatches"
                        description="Invoices found in both sources but with differing amounts"
                        items={report.amount_mismatches.items}
                        color="red"
                        icon={Icons.warning}
                      />
                      <ExceptionsTable
                        title="Missing in Ledger"
                        description="Invoices present on vendor statement but absent from internal AP ledger"
                      ...
                      <ExceptionsTable
                        title="Unapplied Credits"
                        description="Credit notes on vendor statement not deducted in internal ledger"
                ...
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden animate-fade-up">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-100 bg-slate-50/80">
                          {["Invoice ID", "Date", "Amount", "Balance Due"].map(h => (
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {report.amount_mismatches.items.length === 0 &&
                         report.missing_in_ledger.items.length  === 0 &&
                         report.unapplied_credits.items.length  === 0 &&
                          <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-300 text-sm">Loading matched lines…</td></tr>
                        }
                      </tbody>
                    </table>
                  </div>
                  <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/50">
                    <p className="text-xs text-slate-400">
                      Full matched line list available via{" "}
                      <code className="bg-slate-100 rounded px-1 text-slate-500">
                        GET /api/jobs/{report.job_id}/matched
                      </code>
                    </p>
                  </div>
                </div>
